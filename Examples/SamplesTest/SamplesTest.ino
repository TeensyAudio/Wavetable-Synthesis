...
unsigned int AudioSampleCashregister_Interpolate[5809];
unsigned int AudioSampleCashregister_Interpolate1[5809];
...
void setup() {
  ...
  mixer1.gain(2, 0.4);
  mixer1.gain(3, 0.4);
  unsigned int s0 = AudioSampleCashregister[sizeof(AudioSampleCashregister)-1];
  unsigned int s1 = 0;
  float x;
  int x1, x2;

 for (int i = 0; i < (5808/2); i++) {
    s1 = AudioSampleCashregister[i*2];
    AudioSampleCashregister_Interpolate1[i] = s1;
 } 

 for (int i = 0; i < (int)(5808); i++) {
    x = i*.9;
    if (floor(x)== x) {
